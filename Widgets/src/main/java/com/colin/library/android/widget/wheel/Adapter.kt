...
abstract class BaseAdapter<T>(data: List<T>? = null) {
    ...
    internal fun getRealItemCount() = dataList.size
    ...
    fun setDataRange(min: Int, max: Int) {
        if (min < 0 || max < 0 || min > max || !inRange(min, dataList) || !inRange(max, dataList)) {
            isRangeData = false
            rangeDataList.clear()
            return
        }
...
open class ArrayWheelAdapter<T> @JvmOverloads constructor(data: List<T>? = null) :
    ...
    override fun getItemText(item: Any?): String {
        return textFormatter?.formatText(item) ?: (formatter?.invoke(item) ?: (item?.toString()
            ?: ""))
    }
