...
myjs.callback = function (callbackname, response) {
   ...
   console.log("colin_web"+callbackname);
   if (callbackobject !== undefined){
       if(callbackobject.callback != undefined){
          console.log("colin_web"+response);
...
myjs.takeNativeAction = function(commandname, parameters){
    console.log("colin_web takenativeaction")
    var request = {};
    request.name = commandname;
    request.param = parameters;
    if(window.myjs.os.isAndroid){
        console.log("colin_web android take native action" + JSON.stringify(request));
