...
//龙门题库
class QuestionListPracticeActivity :
    BaseActivity<ActivityQuestionListPracticeBinding, UserViewModel>(UserViewModel()) {
    companion object {
        @JvmStatic
        fun start(activity: Activity, type: Int = ConfigApp.TYPE_PRACTICE) {
            val starter = Intent(
                activity, QuestionListPracticeActivity::class.java
            ...
            if (list.get(position).has_power.toInt() == 1) {
                if (list.get(position).questionsCount == 0) {
                    NetErrorPop(mActivity).showPop(
                        getConfigData().home_title4, "哎呀，来早啦", "暂时还不能探索"
                    )
                } else {
