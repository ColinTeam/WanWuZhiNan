...
class DefaultToolbar : ConstraintLayout {
    ...
    private var rightColor: Int = Color.WHITE
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.default_toolbar, 0, 0)
        ...
        backColor = typedArray.getColor(
            R.styleable.default_toolbar_dt_back_color, Color.WHITE
        )
        rightImage = typedArray.getResourceId(R.styleable.default_toolbar_dt_right_image, 0)
        leftImage = typedArray.getResourceId(
            R.styleable.default_toolbar_dt_left_image, R.mipmap.ic_back_mian
        )
        centerColor = typedArray.getColor(
            R.styleable.default_toolbar_dt_center_title_color, centerColor
        )
        rightColor = typedArray.getColor(
            R.styleable.default_toolbar_dt_right_title_color, centerColor
        )
        typedArray.recycle()
        init(context)
    }
    ...
    private fun init(context: Context) {
        mDataBinding = DataBindingUtil.inflate(
            LayoutInflater.from(context), R.layout.default_toolbar, this, true
        )
...
//        mDataBinding!!.ivBack.imageTintList=colorStateList
        ...
        mDataBinding!!.buRight.visibility =
            if (TextUtils.isEmpty(buRightTitle)) View.GONE else View.VISIBLE
        if (rightImage != 0) {
