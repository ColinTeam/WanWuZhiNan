...
class DefaultProtocol : ConstraintLayout {
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        try {
            val typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.default_protocol, 0, 0)
            mTextColor = typedArray.getColor(
                R.styleable.default_protocol_default_protocol_color,
                ResourcesUtil.getColor(context, R.color.color_default)
            )
    ...
    private fun start(title: String) {
        ...
        when (title) {
            getStringExt(R.string.user_privacy) -> {//用户协议
                activity.performLaunchH5Agreement(
                    ConfigApp.USER_AGREEMENT,
                    context.getString(R.string.user_privacy)
                )
            }
            ...
            getStringExt(R.string.privacy_child) -> {//隐私政策
                activity.performLaunchH5Agreement(
                    ConfigApp.PRIVACY_CHILD,
                    context.getString(R.string.privacy_child)
                )
            }
        }
    }
