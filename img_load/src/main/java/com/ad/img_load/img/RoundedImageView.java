 ...
 * Author:ShiMing Shi
 * CreateDate: 2022/6/29 10:36
 * Email:shiming024@163.com
 * Description:圆角矩形图片+边框
 */
...
public class RoundedImageView extends AppCompatImageView {
    ...
    public RoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mType=a.getInt(R.styleable.RoundedImageView_is_filter,0);
        if (index >= 0) {
    ...
    public void loadImg(String url) {
        if (!TextUtils.isEmpty(url)) {
            GlideImgManager.get().clear(this);
            this.setImageDrawable(null);
            getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    GlideImgManager.get().load(RoundedImageView.this, url, width, height);
                }
            });
        }
    }
    ...
    public void loadImg(String url, int position, int key) {
        if (!TextUtils.isEmpty(url)) {
            Object tag = this.getTag(key);
            if (tag != null && (int) tag != position) {
                Glide.with(this).clear(this);
            }
            GlideImgManager.get().loadRoundImg(url, this);
        } else {
            GlideImgManager.get().clear(this);
            this.setImageDrawable(null);
        }
        this.setTag(key, position);
    }


    public void loadImg(int url, int position, int key) {
        if (url > 0) {
            Object tag = this.getTag(key);
            if (tag != null && (int) tag != position) {
                GlideImgManager.get().clear(this);
    ...
    public void loadBlurImg(String url) {
        if (!TextUtils.isEmpty(url)) {
            GlideImgManager.get().loadBlurImg(url, this);
        }
    }

    public void loadBlurImg(String url, int position, int key) {
        if (!TextUtils.isEmpty(url)) {
            Object tag = this.getTag(key);
            if (tag != null && (int) tag != position) {
                GlideImgManager.get().clear(this);
    ...
    public void loadBigPic(String url) {
        if (!TextUtils.isEmpty(url)) {
    ...
    public void setImgFilter(@FloatRange(from = 0.0f, to = 1.0f) float value) {
        ColorMatrix mColorMatrix = new ColorMatrix();
        //0-1
        mColorMatrix.setSaturation(value);
        ColorMatrixColorFilter filter = new ColorMatrixColorFilter(mColorMatrix);
