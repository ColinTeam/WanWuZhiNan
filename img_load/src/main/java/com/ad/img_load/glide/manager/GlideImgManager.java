...
import com.ad.img_load.glide.transform.RoundedTransform;
    ...
    public void loadBlurImg(String url, ImageView img) {
        img.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                loadBlurImg(url, img,width,height);
            }
        });
    }

    private void loadBlurImg(String url, ImageView img,int width,int height) {
        if (TextUtils.isEmpty(url)){
            Glide.with(img)
                    .asBitmap()
                    .load(DEFAULT_ICON)
                    .apply(createOptions().override(width,height).optionalTransform(new BlurTransformation(100)))
                    .into(new SimpleTarget<Bitmap>() {
                        @Override
                        public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                            //Bitmap bitmap = BlurUtils.stackBlur(resource, 180, true);
                            //img.setImageBitmap(blurBitmap(resource,25.0f));
            ...
            Glide.with(img)
                    ...
                    //.apply(createOptions().override(width,height).optionalTransform(new BlurTransformation(0,100)))
                    .into(new SimpleTarget<Bitmap>() {
                        @Override
                        public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                            //Bitmap bitmap = BlurUtils.stackBlur(resource, 180, true);
