...
import com.comm.banner.annotation.Orientation;
import com.comm.banner.config.BannerConfig;
import com.comm.banner.config.IndicatorConfig;
import com.comm.banner.indicator.Indicator;
import com.comm.banner.lifecycle.BannerLifecycleObserver;
import com.comm.banner.lifecycle.BannerLifecycleObserverAdapter;
import com.comm.banner.listener.OnBannerListener;
import com.comm.banner.listener.OnPageChangeListener;
import com.comm.banner.transformer.MZScaleInTransformer;
import com.comm.banner.transformer.ScaleInTransformer;
import com.comm.banner.util.BannerUtils;
import com.comm.banner.util.LogUtils;
import com.comm.banner.util.ScrollSpeedManger;
...
public class Banner<T, BA extends BannerAdapter> extends FrameLayout implements BannerLifecycleObserver {
    ...
    // 指示器相关配置
    private int normalWidth = BannerConfig.INSTANCE.getINDICATOR_NORMAL_WIDTH();
    private int selectedWidth = BannerConfig.INSTANCE.getINDICATOR_SELECTED_WIDTH();
    ...
    private int indicatorGravity = Direction.CENTER;
    ...
    private int indicatorHeight = BannerConfig.INSTANCE.getINDICATOR_HEIGHT();
    private int indicatorRadius = BannerConfig.INSTANCE.getINDICATOR_RADIUS();
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        ...
        normalWidth = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_normal_width, BannerConfig.INSTANCE.getINDICATOR_NORMAL_WIDTH());
        selectedWidth = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_selected_width, BannerConfig.INSTANCE.getINDICATOR_SELECTED_WIDTH());
        ...
        indicatorHeight = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_height, BannerConfig.INSTANCE.getINDICATOR_HEIGHT());
        indicatorRadius = a.getDimensionPixelSize(R.styleable.Banner_banner_indicator_radius, BannerConfig.INSTANCE.getINDICATOR_RADIUS());
        int orientation = a.getInt(R.styleable.Banner_banner_orientation, Orientation.HORIZONTAL);
    ...
    private void initIndicatorAttr() {
        if (indicatorMargin != 0) {
            ...
            IndicatorConfig.Margins margins = new
                    IndicatorConfig.Margins(indicatorMarginLeft, indicatorMarginTop,
    ...
    class BannerOnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            int realPosition = BannerUtils.INSTANCE.getRealPosition(isInfiniteLoop(), position, getRealCount());
    ...
    public Banner setIndicatorPageChange() {
        if (mIndicator != null) {
            int realPosition = BannerUtils.INSTANCE.getRealPosition(isInfiniteLoop(), getCurrentItem(), getRealCount());
    ...
    public Banner setBannerGalleryEffect(int leftItemWidth, int rightItemWidth, int pageMargin, float scale) {
        if (pageMargin > 0) {
            addPageTransformer(new MarginPageTransformer((int) BannerUtils.INSTANCE.dp2px(pageMargin)));
        }
        if (scale < 1 && scale > 0) {
            addPageTransformer(new ScaleInTransformer(scale));
        }
        setRecyclerViewPadding(leftItemWidth > 0 ? (int) BannerUtils.INSTANCE.dp2px(leftItemWidth + pageMargin) : 0,
                rightItemWidth > 0 ? (int) BannerUtils.INSTANCE.dp2px(rightItemWidth + pageMargin) : 0);
        return this;
    }
    ...
    public Banner setBannerGalleryMZ(int itemWidth, float scale) {
        ...
        setRecyclerViewPadding((int) BannerUtils.INSTANCE.dp2px(itemWidth));
        return this;
    }
    ...
    public Banner setIndicatorGravity(@Direction int gravity) {
